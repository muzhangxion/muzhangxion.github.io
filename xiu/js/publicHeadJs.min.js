...
function jsonAllData(a) {
	if(!a) return {};
	var b = "string" == typeof a ? stringToJSON(a) : a;
	return b
}
...
function jsonExist(a) {
	return !!a && "undefined" != typeof a
}

function jsonGet(a) {
	return jsonExist(a) ? a : ""
}
	...
	switch(typeof a) {
		...
		case "object":
			if(a instanceof Array) {
				for(var c = [], d = a.length, e = 0; e < d; e++) c.push(b.jsonToString(a[e]));
				return "[" + c.join(",") + "]"
			}
...
function alterColor() {
	...
	if("http://www.xiu.com/" == a) $(".navlist li a:contains('首页')").addClass("curr").siblings().children().removeClass("curr");
	else {
		var b = a.substring(0, a.indexOf("#s_id") - 1);
		(b.match("http://www.xiu.com") || b.match("http://www.xiu.com/brand") || b.match("http://www.xiu.com/cx")) && $(".navlist li").each(function() {
			var a = $(this).find("a").attr("href"),
				c = a.substring(0, a.indexOf("#s_id") - 1);
			c == b && $(this).find("a").addClass("curr").siblings().children().removeClass("curr")
		})
	}
}

function passSidOnClick() {
	"undefined" == typeof xiuutil && (xiuutil = {});
	var a = window.location.href.toLowerCase(),
		b = xiuutil.getAnchorParam("s_id", a);
	"" == b && (b = xiuutil.getParam("s_id", a)), "" != b && "" == xiuutil.getParam("src", a) && $(function() {
		$("a").live("click", function() {
			var a = ($(this), $.trim($(this).attr("href") || "")),
				c = a.toLowerCase();
			if(0 != c.indexOf("#") && "" == xiuutil.getAnchorParam("s_id", c) && "" == xiuutil.getParam("s_id", c)) {
				var d = $(this).attr("href");
				"javascript:void(0);" != d && "javascript:;" != d && d.indexOf("javascript:") == -1 && $(this).attr("href", xiuutil.setAnchorParam("s_id", b, a))
			}
		})
	})
}! function(a, b) {
	...
	function u(a, c, d, e, f, g) {
		...
		for(var h, i = a[f], j = 0, k = i ? i.length : 0, l = a === fb; j < k && (l || !h); j++) h = i[j](c, d, e), "string" == typeof h && (!l || g[h] ? h = b : (c.dataTypes.unshift(h), h = u(a, c, d, e, h, g)));
	...
	function w(a, b, c, d) {
		if(J.isArray(b)) J.each(b, function(b, e) {
			c || Sa.test(a) ? d(a, e) : w(a + "[" + ("object" == typeof e ? b : "") + "]", e, c, d)
		});
	...
	J.Callbacks = function(a) {
		...
		var d, e, f, g, h, i, j = [],
			...
			l = function(b) {
				var c, d, e, f;
				for(c = 0, d = b.length; c < d; c++) e = b[c], f = J.type(e), "array" === f ? l(e) : "function" === f && (a.unique && n.has(e) || j.push(e))
			},
			m = function(b, c) {
				for(c = c || [], d = !a.memory || [b, c], e = !0, f = !0, i = g || 0, g = 0, h = j.length; j && i < h; i++)
				...
				remove: function() {
					if(j)
						for(var b = arguments, c = 0, d = b.length; c < d; c++)
							for(var e = 0; e < j.length && (b[c] !== j[e] || (f && e <= h && (h--, e <= i && i--), j.splice(e--, 1), !a.unique)); e++);
					return this
				},
				has: function(a) {
					if(j)
	...
	J.extend({
		...
		when: function(a) {
			...
			var d = L.call(arguments, 0),
				...
				j = i.promise();
			if(f > 1) {
				for(; e < f; e++) d[e] && d[e].promise && J.isFunction(d[e].promise) ? d[e].promise().then(b(e), i.reject, c(e)) : --h;
				h || i.resolveWith(i, d)
		...
		data: function(a, c) {
			...
			if(a === b) {
				if(this.length && (l = J.data(j), 1 === j.nodeType && !J._data(j, "parsedAttrs"))) {
					for(g = j.attributes, i = g.length; k < i; k++) h = g[k].name, 0 === h.indexOf("data-") && (h = J.camelCase(h.substring(5)), d(j, h, l[h]));
	...
	J.event = {
			...
			return this instanceof J.Event ? (a && a.type ? (this.originalEvent = a, this.type = a.type, this.isDefaultPrevented = a.defaultPrevented || a.returnValue === !1 || a.getPreventDefault && a.getPreventDefault() ? h : g) : this.type = a, b && J.extend(this, b), this.timeStamp = a && a.timeStamp || J.now(), void(this[J.expando] = !0)) : new J.Event(a, b)
		}, J.Event.prototype = {
		...
		domManip: function(a, c, d) {
			...
			if(this[0]) {
				if(h = i && i.parentNode, e = J.support.parentNode && h && 11 === h.nodeType && h.childNodes.length === this.length ? {
					...
					c = c && J.nodeName(f, "tr");
					for(var k = 0, m = this.length, n = m - 1; k < m; k++) d.call(c ? l(this[k], f) : this[k], e.cacheable || m > 1 && k < n ? J.clone(g, !0, !0) : g)
				}
				...
				if("number" == typeof i && (i += ""), i) {
					if("string" == typeof i)
						if(ua.test(i)) {
							...
							}!J.support.leadingWhitespace && qa.test(i) && p.insertBefore(b.createTextNode(qa.exec(i)[0]), p.firstChild), i = p.childNodes, p && (p.parentNode.removeChild(p), r.length > 0 && (l = r[r.length - 1], l && l.parentNode && l.parentNode.removeChild(l)))
	...
	J.fn.css = function(a, c) {
		...
		style: function(a, c, d, e) {
			if(a && 3 !== a.nodeType && 8 !== a.nodeType && a.style) {
				...
				if(g = typeof d, "string" === g && (f = La.exec(d)) && (d = +(f[1] + 1) * +f[2] + parseFloat(J.css(a, c)), g = "number"), !(null == d || "number" === g && isNaN(d) || ("number" !== g || J.cssNumber[h] || (d += "px"), j && "set" in j && (d = j.set(a, d)) === b))) try {
					i[c] = d
				} catch(k) {}
			}
		},
		...
		J.cssHooks[b] = {
			get: function(a, c, d) {
				if(c) return 0 !== a.offsetWidth ? s(a, b, d) : J.swap(a, Na, function() {
					return s(a, b, d)
				})
			},
			set: function(a, b) {
	...
	Qa = db.exec(Pa.toLowerCase()) || [], J.fn.extend({
		...
		ajax: function(a, c) {
			...
			if(p.promise(w), w.success = w.done, w.error = w.fail, w.complete = q.add, w.statusCode = function(a) {
					...
					return this
				}, m.url = ((a || m.url) + "").replace(Ua, "").replace(Za, Qa[1] + "//"), m.dataTypes = J.trim(m.dataType || "*").toLowerCase().split(bb), null == m.crossDomain && (j = db.exec(m.url.toLowerCase()), m.crossDomain = !(!j || j[1] == Qa[1] && j[2] == Qa[2] && (j[3] || ("http:" === j[1] ? 80 : 443)) == (Qa[3] || ("http:" === Qa[1] ? 80 : 443)))), m.data && m.processData && "string" != typeof m.data && (m.data = J.param(m.data, m.traditional)), u(fb, m, c, w), 2 === v) return !1;
			if(k = m.global, m.type = m.type.toUpperCase(), m.hasContent = !Ya.test(m.type), k && 0 === J.active++ && J.event.trigger("ajaxStart"), !m.hasContent && (m.data && (m.url += ($a.test(m.url) ? "&" : "?") + m.data, delete m.data), e = m.url, m.cache === !1)) {
				...
				m.url = A + (A === m.url ? ($a.test(m.url) ? "&" : "?") + "_=" + z : "")
	...
	J.fn.extend({
		show: function(a, b, c) {
			...
			for(var f = 0, g = this.length; f < g; f++) d = this[f], d.style && (e = d.style.display, J._data(d, "olddisplay") || "none" !== e || (e = d.style.display = ""), ("" === e && "none" === J.css(d, "display") || !J.contains(d.ownerDocument.documentElement, d)) && J._data(d, "olddisplay", E(d.nodeName)));
			for(f = 0; f < g; f++) d = this[f], d.style && (e = d.style.display, "" !== e && "none" !== e || (d.style.display = J._data(d, "olddisplay") || ""));
			return this
		},
		hide: function(a, b, c) {
			if(a || 0 === a) return this.animate(D("hide", 3), a, b, c);
			for(var d, e, f = 0, g = this.length; f < g; f++) d = this[f], d.style && (e = J.css(d, "display"), "none" === e || J._data(d, "olddisplay") || J._data(d, "olddisplay", e));
		...
		custom: function(a, c, d) {
			...
			var f = this,
				g = J.fx;
			this.startTime = rb || B(), this.end = c, this.now = this.start = a, this.pos = this.state = 0, this.unit = d || this.unit || (J.cssNumber[this.prop] ? "" : "px"), e.queue = this.options.queue, e.elem = this.elem, e.saveState = function() {
			...
			}, e() && J.timers.push(e) && !qb && (qb = setInterval(g.tick, g.interval))
		},
		show: function() {
		...
		step: function(a) {
			...
			return h.duration == 1 / 0 ? this.now = e : (c = e - this.startTime, this.state = c / h.duration, this.pos = J.easing[h.animatedProperties[this.prop]](this.state, c, 0, 1, h.duration), this.now = this.start + (this.end - this.start) * this.pos), this.update(), !0
		}
	}, J.extend(J.fx, {
		tick: function() {
...
function(a) {
	a.fn.lazyload = function(b) {
		...
		return "scroll" == c.event && a(c.container).bind("scroll", function(b) {
			...
			var b, c = a || navigator.userAgent.toLowerCase(),
				d = c.match(/msie\D*([\.\d]*)/i);
			return d && d[1] && (b = d[1]), !!/360/.test(c) || +b < 8
		}, this.each(function() {
			var b = this,
				d = isIE67();
			if(d) {
				var e = this.getBoundingClientRect().top,
					...
					h = a(window).height();
				(e < 0 && e > -220 || f >= g && f < h + g) && a(b).attr("src", a(b).attr("original"))
			}
			void 0 == a(b).attr("original") && a(b).attr("original", a(b).attr("src")), "scroll" != c.event || void 0 == a(b).attr("src") || c.placeholder == a(b).attr("src") || a.abovethetop(b, c) || a.leftofbegin(b, c) || a.belowthefold(b, c) || a.rightoffold(b, c) ? (c.placeholder ? a(b).attr("src", c.placeholder) : a(b).removeAttr("src"), b.loaded = !1) : b.loaded = !0, a(b).one("appear", function() {
		...
		if(void 0 === c.container || c.container === window) var d = a(window).height() + a(window).scrollTop();
		else var d = a(c.container).offset().top + a(c.container).height();
		return d <= a(b).offset().top - c.threshold
	}, a.rightoffold = function(b, c) {
		if(void 0 === c.container || c.container === window) var d = a(window).width() + a(window).scrollLeft();
		else var d = a(c.container).offset().left + a(c.container).width();
		return d <= a(b).offset().left - c.threshold
	}, a.abovethetop = function(b, c) {
		if(void 0 === c.container || c.container === window) var d = a(window).scrollTop();
		else var d = a(c.container).offset().top;
		return d >= a(b).offset().top + c.threshold + a(b).height()
	}, a.leftofbegin = function(b, c) {
		if(void 0 === c.container || c.container === window) var d = a(window).scrollLeft();
		...
		a.extend(a.fn, {
			XAutoSuggest: function(b) {
				...
				if("string" == typeof c.url) {
					var d = a(this),
						e = a(this).val();
					e = e ? e : "", d.autocompleteLeon(c.url, {
						max: c.maxNum,
						minChars: c.minChars,
						params: c.params,
						matchContains: !1,
						...
						selectFirst: !1,
						initialValue: e,
						showOnFocus: c.showOnFocus,
						scrollHeight: c.scrollHeight,
						formatItem: function(a, b, c, d, e) {
							var f = "";
							return a.display && (1 == a.type ? f += "<span>" + a.display + '</span><span class="leon-s-oclass">在<span class="leon-fb">' + a.oclassName + "</span>分类下搜索</span>" : (f += "<span>" + a.display + "</span>", a.count && (f += '<span class="leon-s-gn">约' + a.count + "件商品</span>"))), f
						},
						formatResult: function(a) {
							return a.display
						}
					}).result(function(a, b, d) {
						"function" == typeof c.result && c.result(b)
					})
				}
			}
		}), a.fn.extend({
			autocompleteLeon: function(b, c) {
			...
			function d() {
				...
				return t.val(b), j(), t.trigger("result", [a.data, a.value]), !0
			}

			function e(a, b) {
				p == s.DEL && z.hide();
				var d = t.val();
				(b || d != u) && (c.beginChangeDo && c.beginChangeDo(t), u = d, d = g(d), d.length >= c.minChars || 0 == d.length && c.showOnFocus ? (t.addClass(c.loadingClass), c.matchCase || (d = d.toLowerCase()), l(d, k, j)) : (n(), z.hide()))
			}
			...
			function h(d, e) {
				c.autoFill && g(t.val()).toLowerCase() == d.toLowerCase() && p != s.BACKSPACE && (t.val(t.val() + e.substring(g(u).length)), a.Autocompleter.Selection(b, u.length, u.length + e.length))
			}
			...
			function j() {
				var d = z.visible();
				z.hide(), clearTimeout(o), n(), c.mustMatch && t.search(function(a) {
					if(!a)
						if(c.multiple) {
							var b = f(t.val()).slice(0, -1);
							t.val(b.join(c.multipleSeparator) + (b.length ? c.multipleSeparator : ""))
						} else t.val("")
				}), d ? (x = !1, a.Autocompleter.Selection(b, b.value.length, b.value.length)) : x = !0
			}

			function k(a, b) {
				b && b.length && (w || c.showOnFocus && x) ? (n(), z.display(b, a), h(a, b[0].value), z.show()) : j()
			}

			function l(d, e, f) {
				c.matchCase || (d = d.toLowerCase());
				var h = v.load(d);
				if(h && h.length) r = h, e(d, h);
			...
			var o, p, q, r, s = {
				...
				t = a(b).attr("autocomplete", "off").addClass(c.inputClass),
				u = c.initialValue,
				...
				z = a.Autocompleter.Select(c, b, d, y);
			a.browser.opera && a(b.form).bind("submit.autocomplete", function() {
				if(q) return q = !1, !1
			}), t.bind((a.browser.opera ? "keypress" : "keydown") + ".autocomplete", function(b) {
				switch(p = b.keyCode, b.keyCode) {
					case s.UP:
						b.preventDefault(), z.visible() ? z.prev() : e(0, !0);
						break;
					case s.DOWN:
						b.preventDefault(), z.visible() ? z.next() : e(0, !0);
						break;
					case s.PAGEUP:
						b.preventDefault(), z.visible() ? z.pageUp() : e(0, !0);
						break;
					case s.PAGEDOWN:
						b.preventDefault(), z.visible() ? z.pageDown() : e(0, !0);
						break;
					case c.multiple && "," == a.trim(c.multipleSeparator) && s.COMMA:
					case s.TAB:
					case s.RETURN:
					...
					case s.ESC:
						...
						break;
					default:
						a.browser.mozilla || (clearTimeout(o), o = setTimeout(e, c.delay))
				}
			}).focus(function() {
				c.showOnFocus && x && (e(0, !0), x = !1), w++
			}).blur(function() {
				w = 0, x = !0, y.mouseDownOnSelect || i(), "function" == typeof c.blurAutoMatchDo && r && 1 == r.length && r[0].result == f(t.val()) && c.blurAutoMatchDo(r, t)
			}).click(function() {
				w++ > 1 && !z.visible() && e(0, !0)
				...
				function b(a, b) {
					...
					if(b && b.length)
						for(var e = 0; e < b.length; e++)
							...
							"function" == typeof c ? c(d) : t.trigger("result", d && [d.data, d.value])
				}
				var c = arguments.length > 1 ? arguments[1] : null;
				a.each(f(t.val()), function(a, c) {
					l(c, b, b)
				})
			}).bind("flushCache", function() {
				v.flush()
			}).bind("setOptions", function() {
				a.extend(c, arguments[1]), "data" in arguments[1] && v.populate()
			}).bind("unautocomplete", function() {
				z.unbind(), t.unbind(), a(b.form).unbind(".autocomplete")
			}).bind("input", function() {
				a.browser.mozilla && (clearTimeout(o), o = setTimeout(e, c.delay))
			})
			...
			highlight: function(a, b) {
				return a.replace(new RegExp("(?![^&;]+;)(?!<[^<>]*)(" + b.replace(/([\^\$\(\)\[\]\{\}\*\.\+\?\|\\])/i, "\\$1") + ")(?![^<>]*>)(?![^&;]+;)", "i"), "<font>$1</font>")
			},
			scroll: !0,
			scrollHeight: 180
			...
			function c(a, c) {
				...
				var d = a.indexOf(c);
				return "word" == b.matchContains && (d = a.toLowerCase().search("\\b" + c.toLowerCase())), d != -1 && (0 == d || b.matchContains)
			}
			...
			function e() {
				...
				for(var f = 0, g = b.data.length; f < g; f++) {
					var h = b.data[f];
			...
			function i(a) {
				q += a, q < 0 ? q = l.size() - 1 : q >= l.size() && (q = 0)
			}
			...
			function k() {
				o.empty();
				for(var c = j(m.length), d = 0; d < c; d++)
					if(m[d]) {
						var e = b.formatItem(m[d].data, d + 1, c, m[d].value, r);
						if(e !== !1) {
							var f = a("<li/>").html(b.highlight(e, r)).addClass(d % 2 == 0 ? "autosug_even" : "autosug_odd").appendTo(o)[0];
			...
			return {
				...
				show: function() {
					...
					if(n.css({
							...
							top: d.top + c.offsetHeight + b.topOff,
							left: d.left + b.leftOff
...
var url = [];
url.wcsurl = "http://portal.xiu.com/business/", url.cmsurl = "http://www.xiu.com/", url.union = "http://union.xiu.com/", url.portal = "http://my.xiu.com/v3/", url.parm = {
		langId: -7,
		storeId: 10154,
		catalogId: 10101
	}, url.nowDate = Number(new Date), url.random = Math.round(1e4 * Math.random()), url.parms = $.param(url.parm), url.login = url.wcsurl + "XUserCmd?" + url.parms + "&jsoncallback=?", url.checkloginurl = url.wcsurl + "checkloginControlCmd?" + url.parms + "&timeStamp=" + url.nowDate + "&jsoncallback=?", url.loginfloaturl = url.wcsurl + "loginControlCmd?" + url.parms + "&timeStamp=" + url.nowDate + "&jsoncallback=?", url.checkloginControl = url.wcsurl + "checkloginControlCmd?" + url.parms + "&jsoncallback=?", url.jsCheckloginControl = url.wcsurl + "chanelCheckLoginCmd?" + url.parms, url.shoppingCart = url.wcsurl + "XOrderItemCmd?" + url.parms + "&jsoncallback=?", url.shoppingCartNum = url.wcsurl + "XOrderItemAmountCmd?" + url.parms + "&jsoncallback=?", url.searchKey = url.wcsurl + "XAutoSuggest?coreName=MC_10001_CatalogEntry_zh_CN&jsoncallback=?", url.searchForm = "http://search.xiu.com/s?kw=", url.loginHTML = "https://login.xiu.com/?s_id=xiu-login", url.regHTML = "https://login.xiu.com/reg?s_id=xiu-loginreg", url.cartHTML = url.wcsurl + "XOrderItemDisplayCmd?" + url.parms + "&sellType=5", url.cartJson = url.wcsurl + "prdUrlCmd?" + url.parms, url.cartDelete = url.wcsurl + "XOrderItemDeleteForIndexCmd?" + url.parms, url.favoHTML = "/userCenter/favorites.shtml", url.logoutHTML = url.wcsurl + "LogoutCmd?" + url.parms + "&jsoncallback=?", url.detail_jump = url.wcsurl + "PrdDetailURLProcessCmd?" + url.parms + "&jsoncallback=?", url.myXiu = {
		url: "http://my.xiu.com/?s_id=xiu-myxiu",
		list: [{
			name: "我的走秀",
			url: "http://my.xiu.com/?s_id=xiu-myxiu"
		}, {
			name: "我的订单",
			url: "http://my.xiu.com/v3/order/list"
		}, {
			name: "自助退货",
			url: "http://my.xiu.com/v3/order/list?orderCase=5&querySign=2"
		}, {
			name: "我的收藏",
			url: "http://my.xiu.com/v3/favorite/list"
		}, {
			name: "帮助中心",
			url: "http://www.xiu.com/help/index.shtml"
		}, {
			name: "全部吐槽",
			url: "http://my.xiu.com/v3/spitslot/index"
		}]
	},
	function(a) {
		a._xiu_login = function() {}, a._xiu_login.error = '<div style="padding-top:0px;padding-left: 8px;padding-bottom: 8px;"><span>购物袋暂时没有商品!</span></div>', a.extend(a._xiu_login, {
			isLogin: function() {
				return null != a.cookie("xiu.login.userName") && null != a.cookie("xiu.login.tokenId")
			},
			loginInit: function() {
				var b = a("#logindiv"),
					c = b.find("a");
				c.eq(0).attr("url", url.loginHTML), c.eq(1).attr("url", url.regHTML)
			},
			logined: function() {
				var b = a("#logindiv");
				if(null == a.cookie("xiu.login.userName") || null == a.cookie("xiu.login.tokenId")) return a._xiu_login.loginInit();
				var c = a.cookie("xiu.login.userName");
				c.length > 6 && (c = c.substring(0, 6) + "...");
				var d = '<span style="display:inline-block;color:#d7d7d7;vertical-align:top;">您好，</span><a style="font-weight: bold;white-space:nowrap;display:inline-block;overflow:hidden;text-overflow:ellipsis;" href="http://my.xiu.com/v3/">' + c + '</a> <a class="logout" rel="nofollow" href="javascript:$._xiu_login.logout();">[ 退出 ]</a>';
				b.html(d)
			},
			loginCaiBei: function(b) {
				var c = a(".sale_tip"),
					d = a(".login_span"),
					e = a("#logindiv"),
					f = html_decode(b[0].showmsg),
					g = f.indexOf("，") + 1,
					h = f.substring(0, g),
					i = f.substring(g);
				a(".my_caibei").attr("href", decodeURIComponent(b[0].jfUrl)), f.length > 10 ? (f = f.substring(0, 10) + "…", d.html(f)) : d.html(f);
				var j = "您好，" + h + "<b>" + i + '</b>[<a class="logout" rel="nofollow" href="javascript:$._xiu_login.logout();">退出</a>]';
				c.html(b[0].cbHeadShow), e.html(j)
			},
			...
			loginCMS: function() {
				null == a.cookie("sessionID") && a.cookie("sessionID", url.nowDate + "" + url.random, {
					expires: 30,
					path: "/",
					domain: ".xiu.com"
				}), a._xiu_login.loginInit();
				var b = a._xiu_login.error;
				a("#my_bag").html(b), a("#num").html("0"), a(".shopNum").html("0"), a.getJSON(url.checkloginControl, function(b) {
					null != b && null != b[0] && b[0].isAlreadyLogin && a._xiu_login.logined(), a._xiu_login.showCardNum();
					var c = "",
						d = "";
					a.each(url.myXiu.list, function(a, b) {
						0 == a ? d = b.url : c = c + '<a rel="nofollow" href="' + b.url + '" >' + b.name + "</a>"
					}), a("#my_order").attr("href", d), a("#my_xiu").html(c), a("#xiuurl").attr("href", url.myXiu.url)
				})
			},
			logout: function() {
				a.getJSON(url.logoutHTML, function(a) {
					null != a && a.isLogout && (location.href = "http://www.xiu.com")
				})
			},
			showCard: function() {
				a.getJSON(url.shoppingCart, function(b) {
					a._xiu_login.dataProcess(b)
				})
			},
			showCardNum: function() {
				a.getJSON(url.shoppingCartNum, function(b) {
					if(null != b) {
						var c = b.totalQuantity;
						a("#num").html(c > 99 ? "99+" : c), a(".shopNum").html(c > 99 ? "99+" : c)
					}
				})
			},
			removeCart: function(b, c) {
				a.ajax({
					type: "POST",
					url: url.cartDelete + "&skuId=" + c + "&jsoncallback=?",
					dataType: "json",
					timeout: 5e3,
					cache: !1,
					success: function(b) {
			...
			dataProcess: function(b) {
				if("" == b[0].cart || "null" == b[0].cart || "undefined" == b[0].cart || void 0 == b[0].cart || b[0].cart.length < 1) {
					var c = a._xiu_login.error;
					a("#my_bag").html(c), a("#num").html("0"), a(".shopNum").html("0")
				} else {
					var c = '<div id="mybagw" class="bagwarp"><table width="100%" cellspacing="0" cellpadding="0" border="0"><tbody>';
					a.each(b[0].cart, function(a, b) {
						var d = b.catentryId,
							e = b.goodsSn,
							f = b.lineitemtype;
						for(d += ""; d.length < 7;) d = "0" + d;
						c += "<tr>", c += '<td><a rel="nofollow" href="' + url.cartJson + "&goodsId=" + d + "&goodsSn=" + e + "&sellType=" + f + '" target="_blank"><img width="48" height="48" src="' + b.imageUrl + '" onerror="this.src=\'http://www.xiustatic.com/static/img/default/default.50_50.jpg\'"></a></td>', c += "<td>";
						var g = null != b.brandName && "undefined" != b.brandName ? b.brandName : "";
						c += '<p><a rel="nofollow" href="' + url.cartJson + "&goodsId=" + d + "&goodsSn=" + e + "&sellType=" + f + '" title="' + b.catentryName + " " + b.attr + '" target="_blank">' + g + "</br>" + b.catentryName + " " + b.attr + "</a></p>", c += "<span>￥" + b.discountPrice + " x " + b.quantity + "</span>", c += "</td>", c += '<td valign="top"><a class="delex" href="#" rel="nofollow" onclick="$._xiu_login.removeCart(this,\'' + b.skuId + "');return false;\">删除</a></td>", c += "</tr>"
					}), c += "</tbody></table></div>", c += '<div class="total relax">', c += '<span>购物袋中共有<b id="shopNum"> ' + b[0].totalQuantity + " </b>件商品</span>", c += "<span>金额总计：<b>￥" + b[0].totalAmount + "</b></span>", c += '<a rel="nofollow" href="' + url.cartHTML + '">查看购物袋</a>', c += "</div>";
					var d = b[0].totalQuantity;
					a("#my_bag").html(c), a("#num").html(d > 99 ? "99+" : d), a(".shopNum").html(d > 99 ? "99+" : d);
					var e = a("#my_bag tr").length;
					e > 5 ? a("#mybagw").addClass("bagwarp") : a("#mybagw").removeClass("bagwarp")
				}
			}
		})
	}(jQuery), this.XIU || (XIU = {}), XIU.Window = {
		info: {
			t: !0,
			u: window.location.href.replace("http:/", "").replace("https:/", "").toLowerCase(),
			r: document.referrer.replace("http:/", "").replace("https:/", "").toLowerCase()
		},
		data: {
			d: "www.xiu.com",
			s: "200",
		...
		member_visit_time: 0,
		init: function() {
			this.setAllow(), this.getParam(), this.info.u = this.info.u.indexOf("#") > -1 ? this.info.u.substring(0, this.info.u.indexOf("#")) : this.info.u, this.info.r = this.info.r.indexOf("#") > -1 ? this.info.r.substring(0, this.info.r.indexOf("#")) : this.info.r, this.info.anchor = window.location.hash, this.info.u = this.info.u.indexOf("?") > -1 ? this.info.u + this.info.anchor.replace("#", "&") : this.info.u + this.info.anchor.replace("#", "?"), this.info.p = this.getRandom(), this.info.cook_id = 1 == this.checkCookies("cook_id") ? this.getCookies("cook_id") : "", this.info.userId = this.getCookies("xiu.login.userId"), this.info.xiutrack = this.getCookies("xiutrack"), this.info.tuan_userId = this.getCookies("tuan__userId"), this.info.tuan_ruruname = this.getCookies("tuan__ruruname"), this.info.tuan_channelId = this.getCookies("tuan__channelId"), this.info.XResUID = this.getCookies("XResUID"), this.is_head = "/www.xiu.com/" != this.info.u && this.info.u.indexOf("item.xiu.com/product") == -1, this.is_pop = 1 == this.is_new && "" != this.info.cook_id, this.isXiuTuan = this.info.u.indexOf("tuan.xiu.com") > -1;
			var a = this.getCurDate().toString() != (this.info.cook_id.indexOf("@@") > -1 ? this.info.cook_id.split("@@")[1] : "" != this.info.cook_id ? "" : this.getCurDate().toString());
			this.is_first = "" == unescape(this.getCookies("is_new")) && this.is_first, this.is_xiu_old = a && ("undefined" == typeof this.info.userId || "" == this.info.userId) && this.is_first;
			var b = unescape(this.getCookies("is_new"));
			b = "undefined" != b && "" != b && null != b ? b : "";
			var c = b.indexOf("$$") > -1 ? b.split("$$")[1] : this.member_visit_time;
			this.is_first = 0 == c || this.is_first, this.is_login_first = a && "undefined" != typeof this.info.userId && "" != this.info.userId && this.is_first, this.is_xiu_old = 1 != this.is_login_first && this.is_xiu_old, this.is_tuan_old = a && ("undefined" == typeof this.info.tuan_userId || "" == this.info.tuan_userId) && this.is_first, this.is_url_filter = 0 == this.info.u.indexOf("/ferragamo.xiu.com/"), this.is_head && (this.is_pop || this.is_xiu_old || this.is_tuan_old || this.is_login_first) && this.callback();
			var d = "";
			if("" != this.getQueryString("s_id", this.info.r) && "" == this.getQueryString("s_id", this.info.u) && this.info.u.indexOf("/portal.xiu.com/business/") == -1 && this.info.u.indexOf("/my.xiu.com/") == -1 && (d = this.getQueryString("s_id", this.info.r)), a = this.info.u.indexOf("?") > -1 ? this.info.u.substring(this.info.u.indexOf("?") + 1, this.info.u.length) : "", a.indexOf("src") == -1 && "" != d) {
				this.info.u = this.info.u.indexOf("?") > -1 ? this.info.u + "&s_id=" + d : this.info.u + "?s_id=" + d, a = this.info.u.indexOf("?") > -1 ? this.info.u.substring(0, this.info.u.indexOf("?")) : this.info.u;
				var e = a.indexOf("/outlets.xiu.com/") > -1;
				if(e) {
					var f = /[0-9]+/;
					e = a.indexOf("-") == -1 && null != f.exec(a)
				}
				a.indexOf("/product/") != -1 || a.indexOf("/tuan.xiu.com/") != -1 || e || (window.location.href = document.location.protocol + "/" + this.info.u)
			}
		},
		isComputer: function() {
			var a = navigator.userAgent || navigator.vendor || window.opera;
			return isDesktop = !0, (/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino/i.test(a) || /1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|yas\-|your|zeto|zte\-/i.test(a.substr(0, 4))) && (isDesktop = !1), this.ucweb(navigator.userAgent) && (isDesktop = !1), isDesktop
		},
		ucweb: function(a) {
			try {
				return parseFloat(a.match(/ucweb\d+\.\d+/gi).toString().match(/\d+\.\d+/).toString()) >= 8.2
			} catch(b) {
				return a.indexOf("UC") > -1
			}
		},
		callback: function() {
			var a = "",
				b = 86400,
				c = 1;
			this.is_url_filter && this.is_pop ? a = "http://abtest.xiu.com/GetScriptJsonpServlet.js?_pid=9" : this.isXiuTuan ? this.is_pop ? a = "http://abtest.xiu.com/GetScriptJsonpServlet.js?_pid=6" : this.is_tuan_old && (a = "http://abtest.xiu.com/GetScriptJsonpServlet.js?_pid=7", XIU.Window.MakeCookies("is_new", !1, b)) : this.is_pop ? a = "http://abtest.xiu.com/GetScriptJsonpServlet.js?_pid=1&_isnew=" + this.is_new : this.is_xiu_old && this.isComputer() ? (a = "http://abtest.xiu.com/GetScriptJsonpServlet.js?_pid=5", XIU.Window.MakeCookies("is_new", !1, b)) : this.is_login_first && (a = "http://abtest.xiu.com/GetScriptJsonpServlet.js?_pid=14", XIU.Window.MakeCookies("is_new", !1 + "$$" + c, b)), "" != a && $.ajax({
				url: a,
				dataType: "jsonp",
				jsonp: "callbackparam",
				success: function(a) {
					var b = a;
					"undefined" != typeof b && null != b.name && (XIU.Window.request_result = XIU.Window.trim(b.name), MiniSite.JsLoader.load(XIU.Window.info.u, function() {
						MiniSite.JsLoader.popWindow()
					}))
				}
			})
		},
		getCurDate: function() {
			var a = new Date,
				...
				d = a.getDate(),
				e = (a.getDay(), b);
			return e = c > 9 ? e + "-" + c : e + "-0" + c, e = d > 9 ? e + "-" + d : e + "-0" + d
		},
		...
		checkCookies: function(a) {
			var b = this.getCookies(a),
				c = !0;
			if("undefined" == typeof b || "" == b) {
				var d = this.info.p + "@@" + this.getCurDate().toString(),
					e = 15768e4;
				return this.MakeCookies(a, d, e), this.is_new = 1, e = 86400, this.MakeCookies("is_new", !0, e), c
			}
			return c
		},
		MakeCookies: function(a, b, c) {
			var d = new Date;
			d.setTime(d.getTime() + 1e3 * c), document.cookie = a + "=" + escape(b) + "; expires=" + d.toGMTString() + "; path= /; domain=.xiu.com"
		},
		delCooKies: function(a) {
			var b = new Date;
			b.setTime(b.getTime() - 1);
			var c = this.getCookies(a);
			c = null, document.cookie = a + "=" + c + ";domain=.xiu.com;expires=" + b.toGMTString() + "; path=/"
		},
		setParam: function(a) {
			if(a.indexOf("|") > 0)
		...
		getParam: function() {
			if(this.info.t) {
				var a = this.getCookies("temp_param");
				if(a) {
					for(var a = a.split("|"), b = a.length; b--;) {
						var c = a[b].split("="),
							d = c[0] + "=" + c[1];
						this.param.push(d)
					}
					this.delCooKies("temp_param")
				}
			}
		},
		getMakeParam: function() {
			this.param.length > 0 && (this.data.b = this.param.join("|"))
		},
		Random: function() {
		...
		hashClientInfo: function() {
			var a = window.navigator,
				b = window.history.length;
			a = a.appName + a.version + a.language + a.platform + a.userAgent + a.javaEnabled() + a.screen + a.colorDepth + (document.cookie ? document.cookie : "") + (document.referrer ? document.referrer : "");
			for(var c = a[length]; b > 0;) a += b-- ^ c++;
			return this.Hash(a)
		},
		Hash: function(a) {
			var b, c = 1,
		...
		getRandom: function() {
			...
			return a + (this.Random() ^ 2147483647 & this.hashClientInfo()).toString()
		},
		setAllow: function() {
			var a = this.info.u.toLowerCase();
			a.indexOf("pvlog=no") == -1 && /(\w+)\.xiu\.com/.test(window.location.hostname) || (this.info.t = !1)
		},
		decode64: function(a) {
			function b(a) {
				var b, c, d, e, g, h = "",
					...
					k = 0,
					l = /[^A-Za-z0-9\+\/\=]/g;
				l.exec(a) && alert("There were invalid base64 characters in the input text.\nValid base64 characters are A-Z, a-z, 0-9, '+', '/', and '='\nExpect errors in decoding."), a = a.replace(/[^A-Za-z0-9\+\/\=]/g, "");
				do d = f.indexOf(a.charAt(k++)), e = f.indexOf(a.charAt(k++)), g = f.indexOf(a.charAt(k++)), j = f.indexOf(a.charAt(k++)), b = d << 2 | e >> 4, c = (15 & e) << 4 | g >> 2, i = (3 & g) << 6 | j, h += String.fromCharCode(b), 64 != g && (h += String.fromCharCode(c)), 64 != j && (h += String.fromCharCode(i)), b = c = i = "", d = e = g = j = ""; while (k < a.length);
				return unescape(h)
			}
			...
			function c(a) {
				...
				for(var b = 0, c = a.length; b < c; b++) {
					...
					b = f + 1;
					var g = a.slice(b, b + 2),
						h = (parseInt(g, 16), a.slice(b, b + 2), e(a.slice(b, b + 2)));
					switch(h) {
						case 1:
							var i = a.slice(b, b + 2);
							j = String.fromCharCode(parseInt(i, 16)), a = [a.slice(0, b - 1), j, a.slice(b + 2)].join(""), b -= 1;
							break;
						case 2:
							var i = a.slice(b + 2, b + 5);
							if(/%../.test(i)) {
								var j = a.slice(b - 1, b + 5);
								j = d(j), a = [a.slice(0, b - 1), j, a.slice(b + 5)].join(""), b -= 1
							}
							break;
						case 3:
							var i = a.slice(b + 2, b + 8);
							if(/\%..\%/.test(i)) {
								var j = a.slice(b - 1, b + 8);
								j = d(j), a = [a.slice(0, b - 1), j, a.slice(b + 8)].join(""), b -= 1
							}
					}
				}
				return a
			...
			function d(a) {
				...
				for(var b = a.split("%"), c = 0, d = b.length; c < d; c++) {
					b[c] = parseInt(b[c], 16).toString(2);
					var e = b[c].indexOf("0");
					b[c] = b[c].slice(e + 1)
				}
				var f = b.join(""),
			...
			function e(a) {
				...
				var b = parseInt(a, 16),
					c = iUBound = 0;
				return c = 0, iUBound = 127, b >= c && b <= iUBound ? 1 : (c = 194, iUBound = 223, b >= c && b <= iUBound ? 2 : (c = 224, iUBound = 239, b >= c && b <= iUBound ? 3 : (c = 240, iUBound = 244, b >= c && b <= iUBound ? 4 : 0)))
			}
			var f = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";
			return b(c(a))
		},
		getChannelLev2: function() {
			var a = decodeURIComponent(this.info.u),
				b = a.indexOf("?") > -1 ? a.substring(a.indexOf("?") + 1, a.length) : "";
			return b.indexOf("channel_lev1") > -1 ? b.split("&")[b.split("&").length - 1] : ""
		},
		getMakeUrl: function(a) {
			var b = a.d,
				c = this.info.u,
				d = this.info.xiutrack,
				e = this.info.userId,
				f = this.info.tuan_userId,
				g = this.info.tuan_channelId,
				h = this.info.tuan_ruruname,
				i = this.info.XResUID,
				j = this.info.r,
				k = this.info.p,
				l = a.ver,
				...
				p = "",
				q = "",
				r = c.indexOf("?") > -1 ? c.substring(0, c.indexOf("?")) : c,
				s = r.indexOf("/item.xiu.com/product/") > -1,
				t = r.indexOf("/ebay.xiu.com/product/") > -1,
				u = r.indexOf("/outlets.xiu.com/") > -1;
			if(u) {
				var v = /[0-9]+/;
				u = r.indexOf("-") == -1 && null != v.exec(r)
			}
			var w = r.indexOf("/ferragamo.xiu.com/") > -1;
			if(w) {
				var v = /[0-9]+/;
				w = r.indexOf("-") == -1 && null != v.exec(r)
			}
			if("undefined" != typeof o && o.indexOf("xiuitemid") > -1 && (t || s || u || w)) {
				r = j.indexOf("?") > -1 ? j.substring(0, j.indexOf("?")) : j, r = "" == r ? c.indexOf("?") > -1 ? c.substring(0, c.indexOf("?")) : c : r, p = r.indexOf("/ebay.xiu.com/") > -1 ? "ebay" : r.indexOf("/outlets.xiu.com/") > -1 ? "outlets" : r.indexOf("/ferragamo.xiu.com/") > -1 ? "ferragamo" : "xiu", r = j.indexOf("?") > -1 ? j.substring(0, j.indexOf("?")) : j;
				var x = "";
				if(r.indexOf("/cx/") > -1 || r.indexOf("/ebay.xiu.com/match/") > -1 || r.indexOf("/ebay.xiu.com/listpage/") > -1) {
					x = "cx", r.indexOf("/ebay.xiu.com/match/") > -1 ? x = "match" : r.indexOf("/ebay.xiu.com/listpage/") > -1 && (x = "listpage");
					var v = /[0-9]+/;
					q = v.exec(r), q = null != q ? x + "=" + q : "";
				} else if(r.indexOf("/ebay.xiu.com/brand/") > -1 || r.indexOf("/ebay.xiu.com/list/") > -1 || r.indexOf("/brand.xiu.com/") > -1 || r.indexOf("/list.xiu.com/") > -1) {
					x = "brand", (r.indexOf("/list/") > -1 || r.indexOf("/list.xiu.com/") > -1) && (x = "list");
					var v = /[0-9]+/;
					q = v.exec(r), r.indexOf("-") > -1 && (r = r.substring(r.indexOf("-") + 1, r.length), "-" != r.substring(0, 1) && (q = null != v.exec(r) ? q + "-" + v.exec(r) : q)), q = null != q ? x + "=" + q : ""
				} else if(r.indexOf("/outlets.xiu.com/") > -1) {
					x = "outlets";
					var v = /[0-9]+/;
					q = v.exec(r), q = null != q ? x + "=" + q : ""
				} else j.indexOf("/ebay.xiu.com/search/s?tags=") > -1 ? (x = "tag", q = this.getQueryString("tags", j), q = null != q ? x + "=" + q : "") : j.indexOf("/search/s?kw=") > -1 || j.indexOf("/search.xiu.com/s?kw=") > -1 ? (x = "kw", q = this.getQueryString("kw", j), q = null != q ? x + "=" + q : "") : j.indexOf(".shtml") > -1 && (j.indexOf("/ferragamo.xiu.com/woman-") > -1 || j.indexOf("/ferragamo.xiu.com/man-") > -1) ? (x = "ferragamo", q = j.indexOf("/man-") > -1 ? j.substring(j.indexOf("/man-") + 1, j.indexOf(".shtml")) : j.substring(j.indexOf("/woman-") + 1, j.indexOf(".shtml")), q = x + "=" + q) : (r = j.indexOf("?") > -1 ? j.substring(j.indexOf("?"), j.length) : "", r.indexOf("xres_id") > -1 ? (x = "走秀推荐", q = this.getQueryString("xres_id", j), q = x + "=" + q) : r.indexOf("req_id") > -1 && (x = "百分点推荐", q = x + "=百分点推荐"));
				if("" != p || "" != q) {
					var y = "channel_lev1=" + p + "&" + q;
					c = c.indexOf("?") > -1 ? c + "&" + y : c + "?" + y, this.info.u = c
				}
			}
			var z = [];
			if("" != n && z.push("xiutrackid=" + n), "undefined" != e && "" != e && null != e && z.push("userId=" + e), "undefined" != d && "" != d && null != d) {
				var A = d.split(",");
				z.push("channel=" + A[0]), z.push("uName=" + A[1])
			}
			"undefined" != f && "" != f && null != f && (f = this.decode64(f), z.push("tuan_userId=" + f)), "undefined" != g && "" != g && null != g && (g = this.decode64(g), z.push("tuan_channelId=" + g)), "undefined" != h && "" != h && null != h && (h = this.decode64(h), z.push("tuan_ruruname=" + h)), "undefined" != i && "" != i && null != i && z.push("XResUID=" + i), 1 == this.is_new && z.push("is_new=" + this.is_new), z.push("resolution=" + window.screen.width + "*" + window.screen.height);
			var B = "";
			return "undefined" == typeof o ? (o = "", "undefined" != this.info.tunnel_info && "" != this.info.tunnel_info && null != this.info.tunnel_info && (o = o + "tunnel_info=" + this.info.tunnel_info), B = "d={" + b + "}&c={" + z.join("|") + "}&p={" + k + "}&b={" + o + "}&u={" + c + "}&r={" + j + "}&s={" + m + "}&ver={" + l + "}") : ("undefined" != this.info.tunnel_info && "" != this.info.tunnel_info && null != this.info.tunnel_info && (o = o + "|tunnel_info=" + this.info.tunnel_info), B = "d={" + b + "}&c={" + z.join("|") + "}&p={" + k + "}&b={" + o + "}&u={" + c + "}&r={" + j + "}&s={" + m + "}&ver={" + l + "}"), this.checkCharacters(B)
		},
		getNOPageUrl: function(a) {
			var b, c = a.d,
				d = a.project,
				e = this.info.p,
				f = this.info.userId,
				g = this.info.xiutrack,
				h = this.info.tuan_userId,
				i = this.info.tuan_channelId,
				j = this.info.tuan_ruruname,
				k = this.info.XResUID,
				l = this.info.u,
				m = a.ver,
				n = this.info.cook_id,
				o = this.data.val,
				p = [];
			if("" != n && p.push("xiutrackid=" + n), "undefined" != f && "" != f && null != f && p.push("userId=" + f), "undefined" != g && "" != g && null != g) {
				var q = g.split(",");
				p.push("channel=" + q[0]), p.push("uName=" + q[1])
			}
			return "undefined" != h && "" != h && null != h && (h = this.decode64(h), p.push("tuan_userId=" + h)), "undefined" != i && "" != i && null != i && (i = this.decode64(i), p.push("tuan_channelId=" + i)), "undefined" != j && "" != j && null != j && (j = this.decode64(j), p.push("tuan_ruruname=" + j)), "undefined" != k && "" != k && null != k && p.push("XResUID=" + k), "undefined" == typeof d ? (d = "", b = "d={" + c + "}&c={" + p.join("|") + "}&pn={" + d + "}&p={" + e + "}&val={" + o + "}&surl={" + l + "}&ver={" + m + "}") : b = "d={" + c + "}&c={" + p.join("|") + "}&pn={" + d + "}&p={" + e + "}&val={" + o + "}&surl={" + l + "}&ver={" + m + "}", this.checkCharacters(b)
		},
		checkCharacters: function(a) {
			return a.indexOf("#") > 0 ? a.replace(new RegExp(/(#)/g), "") : a
		},
		...
		trim: function(a) {
			return null == a ? "" : (a = a.replace(/^\s*(.*?)[\s\n]*$/g, "$1"), a.replace(/\r\n/g, ""))
		},
		getTunnelInfo: function() {
			var a = this.info.u,
				b = a.indexOf("?") > -1 ? a.substring(a.indexOf("?"), a.length) : "";
			this.info.tunnel_info = unescape(this.getCookies("tunnel_info"));
			var c = "",
				d = "",
				e = this.getQueryString("utm_content", b);
			if(b.indexOf("utm_campaign") > -1 && b.indexOf("utm_source") > -1) c = this.getQueryString("utm_campaign", b), d = this.getQueryString("utm_source", b);
			else {
				var f = this.info.r;
				if(0 == f.indexOf("/cps.xiu.com")) {
					var g = f.indexOf("?") > -1 ? f.substring(f.indexOf("?"), f.length) : "";
					g.indexOf("cps") > -1 && (c = "cps", d = this.getQueryString("cps", g))
				}
			}
			if("" != c && "" != d) {
				"" == this.info.tunnel_info ? this.info.tunnel_info = c + ":" + d + ":" + e + ":" + this.getCurDate() : this.info.tunnel_info += "@@" + c + ":" + d + ":" + e + ":" + this.getCurDate();
				var h = this.info.tunnel_info.split("@@").length - 1;
				h > 4 && (this.info.tunnel_info = this.info.tunnel_info.substring(this.info.tunnel_info.indexOf("@@") + 2, this.info.tunnel_info.length));
				var i = 2592e3;
				this.MakeCookies("tunnel_info", this.info.tunnel_info, i)
			}
		},
		send: function() {
			this.is_head || !(this.is_pop || this.is_xiu_old || this.is_tuan_old || this.is_login_first) || this.is_url_filter || this.callback(), this.info.t && (this.getTunnelInfo(), this.getMakeParam(), jQuery.getScript(document.location.protocol + "//dmtrack.xiu.com/?" + this.getMakeUrl(this.data)))
		},
		straceClick: function(a, b) {
			"" != a && "undefined" != typeof a && (this.data.project = b, this.info.r = this.info.u, this.data.val = a, jQuery.getScript(document.location.protocol + "//spck.xiu.com/?" + this.getNOPageUrl(this.data)))
		},
		ctraceClick: function(a, b) {
			if("" != a && "undefined" != typeof a && "" != b && "undefined" != typeof b) {
				this.data.project = b, this.info.r = this.info.u, this.data.val = a;
				var c = "Microsoft Internet Explorer";
				navigator.appName == c ? jQuery.getScript(document.location.protocol + "//xpck.xiu.com/?" + this.getNOPageUrl(this.data)) : $.ajax({
					async: !1,
					cache: !0,
					type: "GET",
					url: document.location.protocol + "//xpck.xiu.com/?" + this.getNOPageUrl(this.data),
					dataType: "jsonp",
					success: function() {
						return !0
					}
...
MiniSite.Browser = {
	...
	writeDivInfo: function(a, b, c, d, e, f, g) {
		try {
			var h = document.createStyleSheet();
			h.cssText = "body{background-image:url(about:blank);background-attachment:fixed;}"
		} catch(i) {}
		e = e.substring(0, e.indexOf("px;"));
		var j = document.documentElement.clientHeight - e;
		j = (e >= 150 ? Math.ceil(j / 2) : j) + "px;", e += "px;";
		var k = "http://www.xiustatic.com/static/yingyan/1398839666808.png",
			l = '<div align="center" id="shutter" style="background-color:#transparent;filter:progid:DXImageTransform.Microsoft.gradient(enabled=true,startColorstr=#7F000000,endColorstr=#7F000000);background:rgba(0,0,0,0.5);position:fixed;z-index:99999;top:0;left:0;width:100%;height:100%;_position:absolute;_top:expression(document.documentElement.scrollTop+document.documentElement.clientHeight-this.offsetHeight);overflow:hidden;"><div style="margin:' + j + " auto;width:" + f + "height:" + e + '"><a target="_blank" href="' + c + '"><img style="border:none;" src="' + d + '" onclick="XIU.Window.ctraceClick(\'atype=clk' + g + '\',\'eagle_eye\');" /></a><a id="clkclose" style="border:0px;float:right;position:relative;width:71px;height:71px;left:-' + a + "top:-" + b + "z-index:999;background:url('" + k + "') repeat scroll 0% 0% transparent;\"></a></div></div>";
		return l
	},
	writeSpecialDiv: function() {
		var a = (document.documentElement.clientHeight - 392) / 2 + "px",
			b = "http://www.xiustatic.com/static/yingyan/close.png",
			c = '<div align="center" id="shutter" style="background-color:#transparent;filter:progid:DXImageTransform.Microsoft.gradient(enabled=true,startColorstr=#7F000000,endColorstr=#7F000000);background:rgba(0,0,0,0.5); position:fixed;z-index:99999;top:0;left:0;width:100%;height:100%;_position:absolute;_top:expression(document.documentElement.scrollTop+document.documentElement.clientHeight-this.offsetHeight);overflow:hidden;"><div style="position: relative; margin:' + a + ' auto; width:800px; height:392px;"><img src="http://www.xiustatic.com/static/yingyan/eye.png" width="800" height="392" usemap="#eagleEyeMap"><map name="eagleEyeMap"><area onclick="XIU.Window.ctraceClick(\'atype=clk|abproject_id=1|abitem_id=1|abverid=2\',\'eagle_eye\');" shape="rect" coords="189,404,392,449" href="https://login.xiu.com/reg" target="_blank"><area shape="rect" coords="205,281,370,323" href="https://login.xiu.com/reg" target="_blank"><area shape="rect" coords="448,280,613,324" href="http://www.xiu.com/about/xiu_superiority.shtml" target="_blank"></map><a id="clkclose" style="background: url(\'' + b + '\') no-repeat center center; position: absolute; left: -36px; top:-18px; width:32px; height:32px; cursor: pointer;"></a><a href="http://www.xiu.com" style="background: url(\'http://www.xiustatic.com/static/yingyan/eye_btn.jpg\') no-repeat center center; position: absolute; left: 320px; bottom:-60px; width:187px; height:36px; cursor: pointer;"></a></div></div>';
		return c
	},
	writeAutoImage: function(a, b, c, d) {
		try {
			var e = document.createStyleSheet();
			e.cssText = "body{background-image:url(about:blank);background-attachment:fixed;}"
		} catch(f) {}
		for(var g = '<div align="center" id="shutter" style="background-color:#transparent;position:fixed;z-index:88888888;bottom:0;left:0;width:100%;height:100px;_position:absolute;_top:expression(document.documentElement.scrollTop+document.documentElement.clientHeight-this.offsetHeight);overflow:hidden;"><div id="autoimage" name="autoimage" style="width:950px;" overflow:hidden">', h = 0; h < a.split("|").length; h++) g = g + '<a target="_blank" href="' + a.split("|")[h] + '"><img style="border:none;" src="' + b.split("|")[h] + '" onclick="XIU.Window.ctraceClick(\'atype=clk' + d + "','eagle_eye');\"/></a>";
		return g += '<a id="clkclose" style="border:0px;float:right;position:relative;width:15px;height:15px;left:0px;top:-100px;z-index:999;background:url(\'http://www.xiu.com/static/css/default/img/closex.png\') repeat scroll 0% 0% transparent;"></a></div></div>'
	},
	load: function(a, b) {
		var c = document.createElement("script");
		c.setAttribute("charset", "gb2312"), c.setAttribute("type", "text/javascript"), document.getElementsByTagName("head")[0].appendChild(c), MiniSite.Browser.ie ? c.onreadystatechange = function() {
			"loaded" != this.readyState && "complete" != this.readyState || b()
		} : b()
	},
	popWindow: function() {
		var a = "https://login.xiu.com/reg",
			b = "http://www.xiustatic.com/static/yingyan/eagle_eye2.jpg",
			c = "100px;",
			d = "950px;",
			e = "455px;",
			f = "150px;",
			g = XIU.Window.request_result,
			h = "1",
			i = "1",
			j = "1";
		if(g = XIU.Window.trim(g), g.indexOf("_src") > -1 && g.indexOf("_href") > -1 && g.indexOf("_height") > -1) {
			var k = g.substring(g.indexOf("_src="), g.indexOf("?"));
			b = k.indexOf("&") > -1 ? k.substring(k.indexOf("_src=") + 5, k.indexOf("&")) : k.substring(k.indexOf("_src=") + 5, k.length), k = g.indexOf("?abtest=") > -1 ? g.substring(g.indexOf("_href="), g.indexOf("?abtest=")) : g.substring(g.indexOf("_href="), g.length), a = k.substring(k.indexOf("_href=") + 6, k.length), k = g.substring(g.indexOf("_height="), g.indexOf("?")), c = k.indexOf("&") > -1 ? k.substring(k.indexOf("_height=") + 8, k.indexOf("&")) : k.substring(k.indexOf("_height=") + 8, k.length), d = g.indexOf("_width") > -1 ? g.substring(g.indexOf("_width=") + 7, g.indexOf(";") + 1) : d, e = g.indexOf("_closeH") > -1 ? g.substring(g.indexOf("_closeH=") + 8, g.indexOf("||")) + ";" : e, f = g.indexOf("_closeW") > -1 ? g.substring(g.indexOf("_closeW=") + 8, g.indexOf("##")) + ";" : f, h = XIU.Window.getQueryString("abproject_id", g), i = XIU.Window.getQueryString("abitem_id", g), j = XIU.Window.getQueryString("abverid", g)
		}
		var l = XIU.Window.getQueryString("isFromEbay", a.split("|")[0]);
		"true" == l && $.cookie("isFromEbayAd", "Y", {
			domain: ".xiu.com",
			path: "/"
		});
		var m = "|abproject_id=" + h + "|abitem_id=" + i + "|abverid=" + j;
		$(document).ready(function() {
			XIU.Window.is_login_first && $("body").append(MiniSite.JsLoader.writeAutoImage(a, b, c, m)), $("#clkclose").click(function() {
				$("#shutter").remove(), XIU.Window.ctraceClick("atype=close" + m, "eagle_eye")
			})
		}), XIU.Window.is_login_first && $(function() {
			function a(a) {
				$("#autoimage img").eq(a).show(), $("#autoimage img").fadeOut(1e3), $("#autoimage img").eq(a).hide(), $("#autoimage img").eq(a).fadeIn(1e3)
			}

			function b() {
				d == c ? d = 0 : (a(d), d += 1)
			}
			var c = $("#autoimage img").length,
				d = 1;
			setInterval(b, 5e3)
		}), XIU.Window.ctraceClick("atype=exposure" + m, "eagle_eye")
	}
}, XIU.Window.init();
var xiuutil = {
	getAnchorParam: function(a, b) {
		var c = new RegExp("\\#(?:.+&)?" + a + "=(.*?)(?:&.*)?$"),
			d = b.match(c);
		return d ? d[1] : ""
	},
	setAnchorParam: function(a, b, c) {
		var d = c;
		return d = d.indexOf("#") > -1 ? d + "&" + a + "=" + b : d + "#" + a + "=" + b
	},
	correctURL: function(a) {
		if((a.indexOf("#") > -1 || a.indexOf("?") > -1) && a.indexOf("html") == -1 && a.indexOf("shtml") == -1) {
			var b = a.indexOf("?") > -1 ? a.indexOf("?") : a.indexOf("#"),
				c = a.substring(0, b),
				d = a.substring(b);
			return "/" != c.substring(c.length - 1) && (c += "/"), c + d
		}
		return a
	},
	...
	setParam: function(a, b, c) {
		var d, e, f, g;
		return c.indexOf("?") > -1 ? (d = c.indexOf("?"), e = c.substring(0, d + 1), f = c.substring(d + 1), g = e + a + "=" + b + "&" + f) : c.indexOf("#") > -1 ? (d = c.indexOf("#"), e = c.substring(0, d), f = c.substring(d), g = e + "?" + a + "=" + b + f) : g = c + "?" + a + "=" + b, g
	}
};
$(document).ready(function() {
	passSidOnClick(), alterColor()
}), Namespace = {}, Namespace.register = function(a) {
	if("string" == typeof a)
		for(var b = a.split("."), c = c || window, d = 0; d < b.length; d++) c[b[d]] || (c[b[d]] = {}), c = c[b[d]]
};
var getBrowserCSSPrefix = function() {
	return ["-moz-", "-webkit-", "-o-", "-ms-", ""]
};